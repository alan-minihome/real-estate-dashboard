...
export default function VerdictBadge({ overallPass, buySignal, signalReason, size = 'md' }: Props) {
  ...
  if (pass && signal) {
    const strength = parseStrength(signalReason)
    const strengthStyle: Record<string, string> = {
      강: 'bg-emerald-600 text-white',
      중: 'bg-blue-500 text-white',
      약: 'bg-amber-400 text-white',
    }
    return (
      <span
        title={signalReason ? `매수 후보: ${signalReason}` : '매수 후보 (기준통과 + 가격매력)'}
        className={`${padding} ${text} rounded-full font-medium bg-emerald-100 text-emerald-700 border border-emerald-300 whitespace-nowrap inline-flex items-center gap-1`}
      >
        🎯 매수 후보
        {strength && (
          <span className={`text-[9px] font-bold px-1 py-px rounded-sm leading-none ${strengthStyle[strength]}`}>
            {strength}
          </span>
        )}
      </span>
