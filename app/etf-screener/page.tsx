...
function ColTooltip({ label, children }: { label: React.ReactNode; children: React.ReactNode }) {
  ...
  return (
    <span className="relative inline-flex items-center gap-0.5 cursor-help whitespace-nowrap"
      onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      {label}
      <span className="text-[9px] text-slate-400 hover:text-blue-500 leading-none">❓</span>
      {show && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-50 pointer-events-none">
          <div className="w-2 h-2 bg-[#1E293B] rotate-45 mx-auto mb-[-4px]" />
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-[#E2E8F0] sticky top-0 z-20">
              ...
              <th className="text-center px-2 py-3 font-medium text-[#64748B] text-xs whitespace-nowrap">카테고리</th>
              <th className="text-right px-2 py-3 font-medium text-[#64748B] text-xs whitespace-nowrap">
            ...
            {evaluated.filter(r => showAll || r.overallPass).map(({ etf, results, overallPass, passCount, totalActive }, i) => {
              ...
              return (
                <tr key={etf.ticker} className={`border-b border-[#E2E8F0] last:border-0 ${rowBg}`}>
                  ...
                  {/* ETF명 */}
                  <td className="px-3 py-3 text-xs text-[#0F172A] max-w-[150px] overflow-hidden" title={etf.name}>
                    <span className="block truncate">{etf.name}</span>
                  </td>
                  {/* 카테고리 */}
                  <td className="px-2 py-3 text-center whitespace-nowrap">
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full whitespace-nowrap ${CATEGORY_COLOR[etf.category] ?? ''}`}>
                  ...
                  <td className="px-2 py-3 text-center whitespace-nowrap">
                    {etf.div_frequency
                      ? <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full whitespace-nowrap ${FREQ_COLOR[etf.div_frequency] ?? ''}`}>{FREQ_LABEL[etf.div_frequency] ?? etf.div_frequency}</span>
                      : <span className="text-slate-300">—</span>}
                  </td>
                  {/* 감배 횟수 */}
