...
export const CRITERIA_CATALOG: CriteriaItem[] = [
  ...
  },
  {
    ...
    enabled: false,
    desc: "잉여현금흐름 ÷ 시가총액 — 밸류에이션 민감 지표 (참고용)",
    detail: "고평가 우량주도 FCF yield가 낮게 나와 배당 안전성 지표로 부적합. FCF 배당성향을 대신 사용하세요.",
    goodSign: "현금 기반 수익률 양호",
    badSign: "FCF yield 낮음 (고평가 또는 현금 부족)",
  },
  {
    key: "fcf_payout_ratio_max",
    label: "FCF 배당성향",
    unit: "%",
    direction: "max",
    default: 80,
    ...
    enabled: true,
    desc: "연간 배당 총액 ÷ FCF — 실제 현금으로 배당을 감당하는가",
    detail: "EPS 기반 배당성향보다 신뢰성이 높습니다. 80% 초과는 FCF만으로 배당을 온전히 감당하기 어렵다는 신호입니다. 100% 초과는 차입으로 배당을 지급하는 상태입니다.",
    goodSign: "FCF로 배당을 안정적으로 감당 가능",
    badSign: "FCF 대비 배당 부담 과중 — 배당 삭감 위험",
  },
  {
    key: "div_yield_min",
    label: "현재 배당률 최소",
